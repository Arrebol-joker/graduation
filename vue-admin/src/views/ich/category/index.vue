<template>
  <div class="app-container">
    ...
    <el-table @row-click="clickRow" ref="table" highlight-current-row
      ...
      <el-table-column label="类别ID" align="center" prop="pcId" />
...
import { listCategory, getCategory, delCategory, addCategory, updateCategory } from "@/api/ich/category"
...
const data = reactive({
  ...
  upload: {
    ...
    // 上传的地址
    url: baseURL + "/ich/category/importData"
  }
})
...
const handleSelectionChange = (selection) => {
  ids.value = selection.map(item => item.pcId)
...
const handleUpdate = (row) => {
  ...
  getCategory(_pcId).then(response => {
    form.value = response.data
    open.value = true
    title.value = "修改项目申报类别"
  })
}
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy.download('ich/category/export', {
    ...queryParams.value
...
const importTemplate = () => {
  proxy.download('ich/category/importTemplate', {
  }, `template_${new Date().getTime()}.xlsx`)
}

/** 导入按钮操作 */
const handleImport = () => {
  upload.value.title = "项目申报类别导入";
